...
Re-verified 2026-07-16: same wipe pattern (`node_modules` + registration). No concatenation bugs found in templates.ts or creativeWorkflowRunner.ts this time. Fix: `pnpm install` → `build:generated` → `build:api` → `runPostMergeSetup()` (auto-triggered via artifact.toml). All 4 services confirmed up: customer-portal landing page renders, ai-platform shows staff login gate (401 pre-login expected), api-server running, mockup-sandbox vite ready.
