/**
 * migrate-portfolio-p2.ts — Run once to apply Sprint P2 DDL.
 * Usage: pnpm --filter @workspace/api-server tsx src/migrate-portfolio-p2.ts
 */
...
CREATE UNIQUE INDEX IF NOT EXISTS ai_service_portfolios_slug_uidx
  ON ai_platform.ai_service_portfolios(slug) WHERE slug IS NOT NULL;

CREATE UNIQUE INDEX IF NOT EXISTS ai_service_portfolios_portfolio_code_uidx
  ON ai_platform.ai_service_portfolios(portfolio_code) WHERE portfolio_code IS NOT NULL;
...
async function migrate() {
  ...
  try {
    console.log("🚀 Running Sprint P2 DDL migration...");
    await client.query(DDL);
    console.log("✅ Migration complete — P2 schema applied successfully.");
  } catch (err) {
