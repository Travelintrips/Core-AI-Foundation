  ...
  if (p["metadata"] !== undefined) {
    ...
    for (const [key, val] of Object.entries(meta)) {
      ...
      if (val !== null && !["string", "number", "boolean"].includes(typeof val)) {
        return { ok: false, error: `metadata value must be string | number | boolean | null` };
      }
    }
  }
...
async function checkAndMarkDedup(eventId: string): Promise<boolean> {
  const existing = await db
    .select({ eventId: aiDiscoveryEventDedupTable.eventId })
    .from(aiDiscoveryEventDedupTable)
    .where(
      and(
        eq(aiDiscoveryEventDedupTable.eventId, eventId),
        gte(aiDiscoveryEventDedupTable.expiresAt, sql`now()`),
      ),
    )
    .limit(1);

  if (existing.length > 0) return true; // duplicate

  const expiresAt = new Date(Date.now() + 24 * 60 * 60 * 1000);
  await db
    .insert(aiDiscoveryEventDedupTable)
    .values({ eventId, expiresAt })
    .onConflictDoNothing();
...
export async function ingestDiscoveryEvent(
  ...
  if (!validation.ok) {
    logger.warn({ err: validation.error }, "[discovery-analytics] event rejected");
    return { accepted: false, duplicate: false, eventId: randomUUID(), error: validation.error };
  }
  ...
  try {
    isDuplicate = await checkAndMarkDedup(payload.eventId);
  } catch (err) {
    logger.warn({ err }, "[discovery-analytics] dedup check failed — treating as new");
  }
...
export async function getFunnelMetrics(opts: {
  ...
  const countMap: Record<string, number> = Object.fromEntries(
    counts.map((r) => [r.eventName, r.sessions]),
  );

  return steps.map((step, i) => {
    const sessions = countMap[step] ?? 0;
    const prevSessions = i === 0 ? sessions : (countMap[steps[i - 1]] ?? 0);
    const conversionRate = prevSessions > 0 ? Math.round((sessions / prevSessions) * 1000) / 10 : null;
    const dropOffRate = conversionRate !== null ? Math.round((100 - conversionRate) * 10) / 10 : null;
    return { step, order: i + 1, sessions, conversionRate, dropOffRate };
  });
}

export interface ConversionMetrics {
  marketplaceVisitors: number;
  goalDiscoveryImpressions: number;
  goalOpenRate: number | null;
  goalToServiceRate: number | null;
...
export async function getConversionMetrics(opts: {
  ...
  const relevantEvents: DiscoveryEventName[] = [
    "marketplace_viewed",
    "goal_discovery_viewed",
    "goal_opened",
    "service_selected_from_goal",
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        lte(aiDiscoveryEventsTable.occurredAt, opts.endDate),
        eq(aiDiscoveryEventsTable.environment, opts.environment),
        sql`event_name = any(${sql.raw(`ARRAY[${relevantEvents.map((e) => `'${e}'`).join(",")}]`)})`,
      ),
    )
    .groupBy(aiDiscoveryEventsTable.eventName);

  const m: Record<string, number> = Object.fromEntries(rows.map((r) => [r.eventName, r.sessions]));

  const rate = (num: number | undefined, den: number | undefined) =>
    den && den > 0 ? Math.round(((num ?? 0) / den) * 1000) / 10 : null;

  return {
    marketplaceVisitors: m["marketplace_viewed"] ?? 0,
    goalDiscoveryImpressions: m["goal_discovery_viewed"] ?? 0,
    goalOpenRate: rate(m["goal_opened"], m["goal_discovery_viewed"]),
    goalToServiceRate: rate(m["service_selected_from_goal"], m["goal_opened"]),
