...
export class LocalAiPatchApprovalError extends Error {
  constructor(
    message: string,
    readonly kind:
      | "NOT_FOUND"
    ...
    super(message);
    this.name = "LocalAiPatchApprovalError";
  }
}

interface AiPatchContext {
  task: AiCodingTask;
...
function safeRepoPath(value: string): string {
  ...
  if (
    ...
    throw new LocalAiPatchApprovalError(
      `Unsafe AI candidate patch path: ${value}`,
      "INVALID_PATCH",
    );
  }
  return normalized;
...
function parsePatchHeaders(patch: string): string[] {
  ...
  for (const line of patch.split("\n")) {
    ...
    if (match[1] !== match[2]) {
      throw new LocalAiPatchApprovalError(
        "Renamed files are not supported by the deterministic AI candidate patch gate.",
        "INVALID_PATCH",
      );
    }
...
async function latestAiPatchContext(taskId: string): Promise<AiPatchContext> {
  const [task] = await db
    .select()
  ...
  if (!task) {
    throw new LocalAiPatchApprovalError("Coding task not found", "NOT_FOUND");
  }
  if (task.status !== "READY_REVIEW") {
    throw new LocalAiPatchApprovalError(
      "Coding task is not awaiting AI candidate patch approval",
      "NOT_READY",
    );
  }
  ...
  if (!orchestratorRun?.logs) {
    throw new LocalAiPatchApprovalError(
      "Completed Coding Orchestrator AI candidate patch was not found",
      "NOT_READY",
    );
  }
  ...
  try {
    ...
    throw new LocalAiPatchApprovalError(
      "Coding Orchestrator AI candidate patch payload is invalid",
      "INVALID_PATCH",
    );
  }
  ...
  if (orchestration?.nextAction !== "REVIEW_AI_PATCH") {
    throw new LocalAiPatchApprovalError(
  ...
  const aiExecution =
    payload.aiExecution &&
    typeof payload.aiExecution === "object" &&
    !Array.isArray(payload.aiExecution)
      ? (payload.aiExecution as Record<string, unknown>)
      : null;
  const aiHandoff =
    payload.aiHandoff &&
  ...
  if (
    aiExecution?.status !== "PROPOSAL_APPLIED" ||
    aiExecution.privilegeEnded !== true ||
    aiExecution.modelInvoked !== true ||
    aiHandoff?.modelInvoked !== true ||
    aiHandoff.gateStatus !== "PRIVILEGE_ENDED" ||
    localExecution?.source !== "AI_PROPOSAL"
  ) {
    throw new LocalAiPatchApprovalError(
      "AI candidate patch is not bound to a completed constrained AI execution",
      "NOT_READY",
    );
  }
  ...
  if (localExecution?.status !== "APPLIED" || localExecution.rolledBack === true) {
    throw new LocalAiPatchApprovalError(
      "AI candidate patch was not successfully produced",
      "NOT_READY",
    );
  }
  ...
  if (!patch || Buffer.byteLength(patch, "utf8") > MAX_PATCH_BYTES) {
    throw new LocalAiPatchApprovalError(
      "AI candidate patch is empty or exceeds the approval size limit",
      "INVALID_PATCH",
    );
  }
  if (
    ...
    throw new LocalAiPatchApprovalError(
      "AI candidate patch contains redacted, binary, added, or deleted file content that cannot pass this gate",
      "INVALID_PATCH",
    );
  }
  ...
  if (!/^[0-9a-f]{40}$/i.test(expectedHeadSha)) {
    throw new LocalAiPatchApprovalError(
      "AI candidate patch is missing a valid analyzed HEAD SHA",
      "INVALID_PATCH",
    );
  }
  ...
  if (changedFiles.length === 0 || changedFiles.length > MAX_CHANGED_FILES) {
    throw new LocalAiPatchApprovalError(
      "AI candidate patch changed-file list is empty or exceeds the approval limit",
      "INVALID_PATCH",
    );
  }
  ...
  if (!sameFiles(changedFiles, patchFiles)) {
    throw new LocalAiPatchApprovalError(
      "AI candidate patch headers do not match the recorded changed-file list",
      "INVALID_PATCH",
    );
  }
...
async function findExistingAiPatchApproval(taskId: string): Promise<AiCodingRun | null> {
  const runs = await db
    .select()
    .from(aiCodingRunsTable)
    .where(eq(aiCodingRunsTable.taskId, taskId))
  ...
  if (!orchestratorRun?.logs) return null;
  ...
  const approval =
    ...
    typeof payload.localPatchApproval === "object" &&
    !Array.isArray(payload.localPatchApproval)
      ? payload.localPatchApproval as Record<string, unknown>
      : null;
  if (
    orchestration?.nextAction !== "RUN_SANDBOX_VERIFICATION" ||
    approval?.source !== "AI" ||
    approval.gateStatus !== "PATCH_VALIDATED"
  ) {
    return null;
  }
  return runs.find(
    (run) => run.agentName === "AI Patch Gate" && run.status === "COMPLETED",
  ) ?? null;
}
...
async function markGateFailed(
  taskId: string,
  runId: string,
  error: LocalAiPatchApprovalError | Error,
): Promise<void> {
  ...
  await db.transaction(async (tx) => {
    ...
    await tx
      ...
      .set({
        status: "READY_REVIEW",
        resultSummary: `AI candidate patch approval failed: ${message}`,
      })
      .where(eq(aiCodingTasksTable.id, taskId));
  });
}

export async function approveAndValidateAiPatch(taskId: string): Promise<AiCodingRun> {
  const existing = await findExistingAiPatchApproval(taskId);
  if (existing) return existing;

  const context = await latestAiPatchContext(taskId);
  ...
  const [run] = await db.transaction(async (tx) => {
    ...
    const alreadyApproved = await findExistingAiPatchApproval(taskId);
  ...
  try {
    ...
    if (!workspace.cleanup) {
      throw new LocalAiPatchApprovalError(
        "AI candidate patch approval only runs against an isolated remote clone",
        "INVALID_PATCH",
      );
    }
    ...
    try {
      ...
      throw new LocalAiPatchApprovalError(
        `AI candidate patch no longer applies cleanly: ${error instanceof Error ? error.message.slice(0, 700) : String(error)}`,
        "INVALID_PATCH",
      );
    }
    ...
    const approvedPayload = {
      ...
      localPatchApproval: {
        ...
        reason: "Deterministic AI candidate patch matched the analyzed HEAD and passed static verification. Repository scripts must pass the sandbox gate before commit approval.",
      ...
      await tx
        ...
        .set({
          ...
          resultSummary:
            ...
            "Human approval recorded. Static verification passed. Ready for network-disabled sandbox verification; nothing was committed or pushed.",
        })
        .where(eq(aiCodingTasksTable.id, taskId));
    ...
    const normalized =
      error instanceof LocalAiPatchApprovalError
        ? error
        : new LocalAiPatchApprovalError(
