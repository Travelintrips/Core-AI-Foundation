...
const SYSTEM_CONSTRAINTS = [
  ...
  "Allowed operation types are replace_text, insert_before, insert_after, delete_text, and create_file only. Every operation file must exactly equal one entry in allowedFiles.",
  "replace_text fields: type, file, oldText, newText, expectedOccurrences. insert_before/insert_after fields: type, file, anchor, content, expectedOccurrences. delete_text fields: type, file, text, expectedOccurrences. create_file fields: type, file, content. Use create_file only when diagnostics state that the exact allowed target does not exist at the approved base SHA.",
