...
type GreenCiContinuationResult =
  | {
      continued: false;
      reason: "BINDING_NOT_GREEN" | "STALE_HEAD_SHA" | "GRAPH_NOT_FOUND";
    }
  | {
      continued: true;
      eventId: string;
...
export async function continueAfterGreenCi(input: {
  bindingId: string;
  eventId: string;
}): Promise<GreenCiContinuationResult> {
  return db.transaction(async (tx) => {
    ...
    if (!binding || binding.state !== "GREEN") {
      return { continued: false, reason: "BINDING_NOT_GREEN" };
    }
    ...
    if (!workstream || workstream.headSha !== binding.headSha) {
      return { continued: false, reason: "STALE_HEAD_SHA" };
    }
    ...
    if (!graph) {
      return { continued: false, reason: "GRAPH_NOT_FOUND" };
    }

    let nextAction = "NO_AUTOMATIC_ACTION";
    if (workstream.status === "COMPLETED") {
      nextAction = "DISPATCH_READY_WORKSTREAMS";
    } else if (workstream.status === "REVIEW_REQUIRED") {
      nextAction = "WAIT_FOR_EXPLICIT_REVIEW";
    } else if (workstream.status === "FAILED") {
      nextAction = "REVIEW_CI_FAILURE";
    } else if (workstream.status === "READY") {
      nextAction = "READY_FOR_EXISTING_WORKER_CLAIM";
    }
    ...
    const checkpoint = {
      ...
      nextAction,
      approvalGatesPreserved: true as const,
    };

    await tx
      .update(aiCodingCiBindingsTable)
      .set({ lastCheckpointJson: checkpoint })
      .where(eq(aiCodingCiBindingsTable.id, binding.id));

    publishSafe({
      eventType: "coding.ci.checkpoint.ready",
...
export async function executeGreenCiNextAction(
  checkpoint: GreenCiContinuationResult,
) {
  if (
    !checkpoint.continued ||
    checkpoint.nextAction !== "DISPATCH_READY_WORKSTREAMS"
  ) {
    return {
      executed: false,
      reason: "NO_BOUNDED_DISPATCH_ACTION" as const,
    };
  }

  const { dispatchReadyCodingWorkstreams } = await import(
    "./localCodingMultiWorkerExecutionService.js"
  );
