...
export async function listPendingCodingBridgeResponses(limit=50){await ensureCodingControlBridgeTables(); return db.select().from(aiCodingBridgeResponsesTable).where(isNull(aiCodingBridgeResponsesTable.acknowledgedAt)).orderBy(asc(aiCodingBridgeResponsesTable.createdAt)).limit(Math.max(1,Math.min(100,limit)));}
export async function acknowledgeCodingBridgeResponse(id:string){await ensureCodingControlBridgeTables(); const [row]=await db.update(aiCodingBridgeResponsesTable).set({acknowledgedAt:new Date()}).where(eq(aiCodingBridgeResponsesTable.id,id)).returning();return row??null;}
...
export async function getCodingBridgeAvailability(clientId:string){await ensureCodingControlBridgeTables(); const [row]=await db.select().from(aiCodingBridgePresenceTable).where(eq(aiCodingBridgePresenceTable.clientId,clientId)).limit(1);if(!row)return{clientId,state:"UNAVAILABLE" as const,leaseExpiresAt:null,lastSeenAt:null};return{clientId,state:row.leaseExpiresAt.getTime()>Date.now()?"ACTIVE" as const:"UNAVAILABLE" as const,leaseExpiresAt:row.leaseExpiresAt,lastSeenAt:row.lastSeenAt};}
