...
router.post("/ai/design-render-batches/:id/export-zip", async (req, res) => {
  try {
    ...
    const { enqueueZipExport, EmptyBatchError } = await import("../services/designZipExportService.js");
    let result: Awaited<ReturnType<typeof enqueueZipExport>>;
    try {
      result = await enqueueZipExport(id, ctx.tenantId);
    } catch (err) {
      if (err instanceof EmptyBatchError) {
        return res.status(422).json({ error: (err as Error).message, code: "EMPTY_BATCH" });
      }
      throw err;
    }
    const { export: zipExport, created } = result;
    ...
    // Only enqueue a worker job when a brand-new export was just created.
    // Returning an existing queued/generating export → do NOT re-enqueue
    // (idempotency: one job per active export record).
    if (created) {
      await enqueue({
...
/**
 * GET /ai/design-zip-exports/:id/download?token=<signed-token>
 *
 * Signed-URL download endpoint for completed ZIP exports.
 * The token is generated by GET /ai/design-render-batches/:id/export-zip
 * and is valid for 1 hour (SIGNED_URL_TTL_SECONDS = 3600).
 *
 * Security properties:
 *   - Token is HMAC-signed (SESSION_SECRET); forgery is infeasible.
 *   - Token carries tenantId; cross-tenant access is rejected.
 *   - Token has exp claim; replay after expiry is rejected.
 *   - zipStoragePath is NEVER returned in the response body — only the
 *     binary stream is sent (storage path stays server-side).
 *   - Endpoint is intentionally outside admin-key auth path so signed
 *     tokens can be used by frontend without re-sending the admin key.
 *     Token alone is the credential for this endpoint.
 */
router.get("/ai/design-zip-exports/:id/download", async (req, res) => {
  try {
    const token = String(req.query["token"] ?? "");
    if (!token) return res.status(400).json({ error: "Missing token", code: "MISSING_TOKEN" });

    const { verifyZipDownloadToken, getZipExport } = await import(
      "../services/designZipExportService.js"
    );

    const verification = verifyZipDownloadToken(token);
    if (!verification.valid) {
      return res.status(401).json({ error: `Invalid token: ${verification.reason}`, code: "INVALID_TOKEN" });
    }

    const exportId = parseInt(String(req.params["id"]), 10);
    if (isNaN(exportId)) return res.status(400).json({ error: "Invalid export ID" });

    // Token exportId must match the URL param (prevents token reuse across exports)
    if (verification.exportId !== exportId) {
      return res.status(401).json({ error: "Token mismatch", code: "TOKEN_MISMATCH" });
    }

    const zipExport = await getZipExport(exportId, verification.tenantId!);
    if (!zipExport) {
    ...
    const { parseStoragePath, downloadBuffer } = await import(
      "../services/designZipExportService.js"
    );

    // Derive a safe filename from batchId
    const filename = `design-batch-${zipExport.batchId}-export-${exportId}.zip`;

    res.setHeader("Content-Type", "application/zip");
    res.setHeader("Content-Disposition", `attachment; filename="${filename}"`);
    if (zipExport.fileSizeBytes) {
      res.setHeader("Content-Length", String(zipExport.fileSizeBytes));
    }
    // Prevent caching of signed download responses
    res.setHeader("Cache-Control", "no-store");

    const buffer = await downloadBuffer(zipExport.zipStoragePath);
