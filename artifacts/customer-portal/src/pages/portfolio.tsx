...
type PublicPortfolio = {
  id: number; serviceId: number; slug: string | null;
  title: string; shortDescription: string | null;
  industry: string; style: string;
  packageLabel: string | null; packageLevel: string | null;
  coverImage: string | null; galleryJson: GalleryItem[] | null;
  ...
  deliveryTime: string | null; rating: string | null;
  views: number; completedProjects: number;
  featured: boolean; isDemo: boolean;
};
...
function BeforeAfterSlider({ before, after, labelBefore, labelAfter }: { before: string; after: string; labelBefore: string; labelAfter: string }) {
  ...
  return (
    <div
      ...
      onMouseMove={(e) => { if (e.buttons !== 1) return; const rect = e.currentTarget.getBoundingClientRect(); setPos(Math.max(0, Math.min(100, ((e.clientX - rect.left) / rect.width) * 100))); }}
      onTouchMove={(e) => { const rect = e.currentTarget.getBoundingClientRect(); setPos(Math.max(0, Math.min(100, ((e.touches[0].clientX - rect.left) / rect.width) * 100))); }}
    >
      <img src={after} alt={labelAfter} className="absolute inset-0 w-full h-full object-cover" draggable={false} />
      <div className="absolute inset-0 overflow-hidden" style={{ clipPath: `inset(0 ${100 - pos}% 0 0)` }}>
        <img src={before} alt={labelBefore} className="w-full h-full object-cover" draggable={false} />
      </div>
      ...
      <span className="absolute top-2 left-2 px-2 py-0.5 rounded-full bg-black/50 text-white text-xs">{labelBefore}</span>
      <span className="absolute top-2 right-2 px-2 py-0.5 rounded-full bg-primary text-primary-foreground text-xs">{labelAfter}</span>
    </div>
  );
}
...
function PortfolioModal({ portfolio, onClose, onStartProject }: {
  portfolio: PublicPortfolio; onClose: () => void; onStartProject: () => void;
}) {
  const { t } = useTranslation();
  return (
    <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-background rounded-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto p-6 md:p-8" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-start justify-between mb-5">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <h3 className="text-xl font-serif font-medium">{portfolio.title}</h3>
              {portfolio.isDemo && <span className="px-2 py-0.5 rounded-full bg-amber-100 text-amber-700 text-xs font-medium">AI Demo</span>}
            </div>
          ...
          <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-muted text-muted-foreground" aria-label={t('portfolio.modal.close')}>
            <X className="w-4 h-4" />
          </button>
        </div>

        {portfolio.beforeImage && portfolio.afterImage ? (
          <BeforeAfterSlider before={portfolio.beforeImage} after={portfolio.afterImage} labelBefore={t('portfolio.before')} labelAfter={t('portfolio.after')} />
        ...
        {portfolio.shortDescription && <p className="text-sm text-muted-foreground leading-relaxed mb-5">{portfolio.shortDescription}</p>}

        {portfolio.deliverablesJson && portfolio.deliverablesJson.length > 0 && (
          <div className="mb-5">
            <p className="text-sm font-medium mb-2">{t('portfolio.modal.deliverables')}</p>
        ...
        {portfolio.workflowJson && portfolio.workflowJson.length > 0 && (
          <div className="mb-5">
            <p className="text-sm font-medium mb-2">{t('portfolio.modal.workflow')}</p>
            <div className="flex flex-wrap gap-2">
        ...
        <div className="flex items-center gap-4 pt-4 border-t border-border text-xs text-muted-foreground mb-5">
          ...
          {portfolio.completedProjects > 0 && <span>{portfolio.completedProjects} projects</span>}
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <button onClick={onStartProject}
            ...
            {t('portfolio.modal.startProject')}
          </button>
...
function FilterBar({ filters, industries, styles, onChange }: {
  filters: Filters; industries: string[]; styles: string[]; onChange: (f: Filters) => void;
}) {
  const { t } = useTranslation();
    ...
    <div className="mb-6">
      <div className="flex items-center gap-2 flex-wrap">
        ...
        >
          ...
          {t('portfolio.filter.all')} {hasActive && <span className="w-4 h-4 rounded-full bg-primary text-primary-foreground text-[10px] flex items-center justify-center">!</span>}
        ...
        {filters.industry && (
          <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-muted border text-xs">
            ...
            <button onClick={() => onChange({ ...filters, industry: "" })} aria-label={t('common.clear')}><X className="w-3 h-3" /></button>
          </span>
        )}
        {filters.style && (
          <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-muted border text-xs">
            {filters.style}
            <button onClick={() => onChange({ ...filters, style: "" })} aria-label={t('common.clear')}><X className="w-3 h-3" /></button>
          </span>
        )}
        {hasActive && (
      ...
      {open && (
        <div className="mt-3 p-4 rounded-2xl border border-border bg-card space-y-4">
          <div>
            <p className="text-xs font-medium text-muted-foreground mb-2">{t('portfolio.filter.industry')}</p>
          ...
          <div>
            <p className="text-xs font-medium text-muted-foreground mb-2">{t('portfolio.filter.style')}</p>
            <div className="flex flex-wrap gap-1.5">
              {styles.map((s) => (
      ...
      <div className="min-h-screen bg-background">
        ...
        <section className="bg-gradient-to-br from-primary/5 via-background to-secondary/5 border-b border-border px-4 pt-16 pb-12">
          <div className="max-w-5xl mx-auto text-center">
            ...
            <h1 className="text-4xl md:text-5xl font-serif font-medium mb-4">{t('portfolio.title')}</h1>
            <p className="text-muted-foreground max-w-2xl mx-auto text-lg mb-8">{t('portfolio.subtitle')}</p>
        ...
        <section className="max-w-6xl mx-auto px-4 py-10">
          <FilterBar filters={filters} industries={filterOptions.industries} styles={filterOptions.styles} onChange={handleFilterChange} />

          {loading ? (
            <div className="flex flex-col items-center justify-center py-24 gap-3 text-muted-foreground">
              <Loader2 className="w-8 h-8 animate-spin" />
              <p className="text-sm">{t('portfolio.loading')}</p>
            </div>
          ) : error ? (
            ...
            <div className="rounded-2xl border border-dashed border-border p-12 text-center">
              ...
              <p className="font-medium mb-1">{t('portfolio.empty')}</p>
              <p className="text-sm text-muted-foreground mb-4">{t('services.empty.desc')}</p>
              <button onClick={() => handleFilterChange({ industry: "", style: "", sort: "featured" })}
            ...
            <>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                {portfolios.map((p) => (
                  <button key={p.id} onClick={() => openPortfolio(p)}
                    ...
                    <div className="aspect-[4/3] bg-muted overflow-hidden relative">
                      ...
                      {p.beforeImage && p.afterImage && (
                        <span className="absolute bottom-3 right-3 px-2 py-0.5 rounded-full bg-black/50 text-white text-xs">
                          {t('portfolio.before')}/{t('portfolio.after')} ↔
                        </span>
                      )}
                    </div>
                    <div className="p-4 space-y-1.5">
                      <p className="font-medium leading-snug">{p.title}</p>
                      {p.shortDescription && <p className="text-xs text-muted-foreground line-clamp-2">{p.shortDescription}</p>}
                      <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <div className="flex items-center gap-3 text-xs text-muted-foreground pt-1">
                        ...
                        {p.completedProjects > 0 && <span>{p.completedProjects} {t('services.card.projects')}</span>}
                      </div>
                    </div>
                  </button>
                ))}
        ...
        <section className="border-t border-border bg-muted/30 px-4 py-16">
          <div className="max-w-2xl mx-auto text-center">
            <h2 className="text-2xl font-serif font-medium mb-3">
              {t('landing.finalCta.title')} {t('landing.finalCta.titleSuffix')}
            </h2>
            <p className="text-muted-foreground mb-8">{t('landing.finalCta.desc')}</p>
        ...
        {activePortfolio && (
          <PortfolioModal
            ...
            onStartProject={() => { setActivePortfolio(null); setLocation("/services"); }}
          />
        )}
      </div>
    </Layout>
