...
interface Props { scene: DesignScene; onChange: (embellishments: Embellishment[]) => void; }
...
export function FashionEmbellishmentEditor({ scene, onChange }: Props) {
  ...
  useEffect(() => {
    if (!parts.some(part => part.id === targetPartId)) setTargetPartId(parts[0]?.id ?? "");
  }, [parts, targetPartId]);

  const add = () => {
    if (!targetPartId || !parts.some(part => part.id === targetPartId)) return;
    const item: Embellishment = { id: crypto.randomUUID(), targetPartId, type, color, sizeMm: Math.max(0.5, sizeMm), density: Math.min(100, Math.max(0, density)), pattern };
  ...
  return <section className="rounded-xl border p-4 space-y-4">
    ...
    <div className="flex gap-3 items-end"><div className="w-52"><Label>Pola</Label><Select value={pattern} onValueChange={v => setPattern(v as Embellishment["pattern"])}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{["free","repeat","mirror","radial","follow-edge"].map(v => <SelectItem key={v} value={v}>{v}</SelectItem>)}</SelectContent></Select></div><Button onClick={add} disabled={!targetPartId}><Plus className="h-4 w-4 mr-1" /> Tambah</Button></div>
    <div className="space-y-2">{(scene.embellishments ?? []).map(e => <div key={e.id} className="flex items-center justify-between rounded-lg border px-3 py-2 text-sm"><span>{e.type} · {scene.objects.find(p => p.id === e.targetPartId)?.name ?? e.targetPartId} · {e.sizeMm}mm · {e.density}% · {e.pattern}</span><Button size="icon" variant="ghost" onClick={() => onChange((scene.embellishments ?? []).filter(x => x.id !== e.id))}><Trash2 className="h-4 w-4" /></Button></div>)}</div>
  </section>;
}
