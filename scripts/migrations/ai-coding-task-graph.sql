...
ALTER TABLE ai_platform.ai_coding_workstreams
  ADD COLUMN IF NOT EXISTS child_task_id UUID REFERENCES ai_platform.ai_coding_tasks(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS child_run_id UUID REFERENCES ai_platform.ai_coding_runs(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS job_id INTEGER REFERENCES ai_platform.ai_jobs(id) ON DELETE SET NULL;
