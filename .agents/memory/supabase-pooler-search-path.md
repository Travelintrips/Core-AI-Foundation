...
**How to apply:** Keep the app's dedicated `ai_platform` schema and run `SET search_path TO ai_platform, public` in `verify`; do not replace it with an unawaited `pool.on("connect")` query. Raw SQL inside transaction-pooler transactions must still qualify tables as `ai_platform.<table>`.