...
/**
 * Team 04 — Service Normalization & Solution Collections (V4.2E / V4.2H)
 *
 ...
 *
 * FK deletion rules (documented per spec requirement):
...
export const serviceNormalizationMappingsTable = appSchema.table(
  "service_normalization_mappings",
  {
    ...
    /** Internal review metadata. NEVER exposed in public API responses. */
...
export const serviceAliasesTable = appSchema.table(
  ...
  {
    id: serial("id").primaryKey(),
    /** FK → service_canonical_concepts.id RESTRICT */
    canonicalConceptId: integer("canonical_concept_id")
      .notNull()
      .references(() => serviceCanonicalConceptsTable.id, { onDelete: "restrict" }),
    /** Raw alias string as submitted by admin (e.g. "Company Profile Design") */
    alias: text("alias").notNull(),
    /** trim().toLowerCase() of alias — used for dedup and lookup */
    normalizedAlias: text("normalized_alias").notNull(),
    /**
     * name            — alternate customer-facing name
     * legacy_code     — old service code string
     * language_variant — Bahasa Indonesia / alternate locale label
     * typo            — common misspelling
     */
    aliasType: text("alias_type").notNull().default("name"),
    /** BCP-47 locale or 'any'. Default 'any' = language-neutral. */
...
export const insertServiceAliasSchema = createInsertSchema(serviceAliasesTable).omit({
  id: true,
  normalizedAlias: true, // computed server-side
  createdAt: true,
});
...
export const solutionCollectionsTable = appSchema.table(
  ...
  {
    id: serial("id").primaryKey(),
    /** Stable code. e.g. "sc_brand_launch" */
    code: text("code").notNull().unique(),
