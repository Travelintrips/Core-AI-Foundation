...
const generatedZodFiles = [];
const collectGeneratedZodFiles = (dir) => {
  if (!fs.existsSync(dir)) return;
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    const entryPath = path.join(dir, entry.name);
    if (entry.isDirectory()) collectGeneratedZodFiles(entryPath);
    else if (entry.isFile() && entry.name.endsWith(".ts")) generatedZodFiles.push(entryPath);
  }
};
collectGeneratedZodFiles(generatedZodDir);
for (const generatedZodFile of generatedZodFiles) {
  ...
  generatedZod = generatedZod
    .replace(/(?:z|zod)\.int\(\)/g, "zod.number().int()")
    .replace(/(?:z|zod)\.float32\(\)/g, "zod.number()")
    .replace(/(?:z|zod)\.float64\(\)/g, "zod.number()")
    .replace(/(?:z|zod)\.uuid\(\)/g, "zod.string().uuid()")
    .replace(/(?:z|zod)\.email\(\)/g, "zod.string().email()")
    .replace(/(?:z|zod)\.url\(\)/g, "zod.string().url()")
    .replace(/(?:z|zod)\.datetime\(\)/g, "zod.string().datetime()")
    .replace(/(?:z|zod)\.date\(\)/g, "zod.string().date()")
    .replace(/(?:z|zod)\.time\(\)/g, "zod.string().time()")
    .replace(/(?:z|zod)\.looseObject\(/g, "zod.object(");
